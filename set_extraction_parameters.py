...
def multiplot(some_cube,some_spec,some_s2n,extract_map,extra_spec,line_wl,file_name,mjd):
    ...
    ax_spec.set_xlabel("Wavelength [Angstrom] MJD="+str(mjd))
    ax_spec.set_ylabel("Counts")
 
    ax_spec.annotate('Extracted Spec.',xy=(0.05,0.6),xycoords='axes fraction',color='red')
    ax_spec.annotate('Sky Spec.',xy=(0.05,0.5),xycoords='axes fraction',color='blue')
...
for epoch in range(0,len(list_of_lists)):
    ...
    for file_loop in range(0,len(fits_list)):
        ...
        plot_out = file_name.split('.')[0]
        multiplot(data,spectrum,signal_noise,extract_pix_map,sky_spectrum,LINE_WL,plot_out,mjd)

#        pix4extraction = int(raw_input("Set pixel number for extraction:"))
        pix4extraction = 20 #int(raw_input("Set pixel number for extraction:"))
        ...
        plot_out = file_name.split('.')[0]+'extraction_test'
        print plot_out
        multiplot(data,spectrum,signal_noise,extract_pix_map,sky_spectrum,LINE_WL,plot_out,mjd)

#        notes.append(raw_input("Notes on epoch:?"))
#        pix_out.append(pix4extraction)
...
#        multiplot(data,data.norm_flux,signal_noise,extract_pix_map,pipe_spec.norm_flux,LINE_WL,file_name)
    ...
    print type(fits_list)
    print type(signal_noise)
    print type(pix_out)
    print type(notes)

#    tmp = file_name.split('/')#,file_name)
#    txt_file = tmp[0]+'_'+tmp[1]+'_extraction.txt'
#    print "file",type(fits_list)
#    data_array = np.array(zip(fits_list,signal_noise,pix_out,notes))
#    print data_array
#    np.savetxt(txt_file,data_array,fmt=['%f']+['%f']+['%f']+['%f'],delimiter='    ')
