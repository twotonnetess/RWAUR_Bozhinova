...
for epoch in range(0,len(list_of_lists)):
    ...
    for file_loop in range(0,len(fits_list)):
        ...
        multiplot(data,spectrum,signal_noise,extract_pix_map,sky_spectrum,LINE_WL,file_name)
       # EW_cube.append(data.ew(6562.1,40))

        if file_loop == 0:
            line_win, contin_wins = data.check_EW_parameters()
            #Estimate EW parameters first time	
        EW_cube.append(data.ew(line_win,contin_wins))

        pipe_spec = SpecCube(file_name,5)
        pipe_spec.get_spectrum("No Header Output")
    
        pipe_spec.normalise_spec(NORM_WL)
        EW_pipe.append(pipe_spec.ew(line_win,contin_wins))

        multiplot(data,data.norm_flux,signal_noise,extract_pix_map,pipe_spec.norm_flux,LINE_WL,file_name)

	date.append(data.return_obs_date())
        quality = raw_input("Well extracted (1) Yes (2) No : ")
        quality_flag.appen(quality)

    tmp = file_name.split('/')#,file_name)
    txt_file = tmp[0]+'_'+tmp[1]+'_EW.txt'
    print "file",type(fits_list)
    print "ew",type(EW_pipe)
    print EW_pipe
    print fits_list
    data_array = np.array(zip(fits_list,EW_pipe,EW_cube))
    print data_array
    np.savetxt(txt_file,data_array,fmt=['%s']+['%s']*2,delimiter='    ')

    EW_parameters = np.array(zip[line_win,contin_wins])

data_array = np.array(zip(date,quality_flag))
print data_array
np.savetxt('quality_of_spectra.txt',data_array,delimiter='    ')

#plt.scatter(EW_pipe,EW_cube)
#plt.savefig('plots/'+file_name+'_EW.png',bbox_inches='tight')

### Notes
## See if the spectra changes from extraction to extraction. 

## Find some sky spectra to remove. 

## Perhaps find the pixels with low signal to noise. and use those 
